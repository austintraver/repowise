...
def test_build_embedder_raises_on_invalid_ollama_timeout(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A misconfigured timeout must surface, not silently become MockEmbedder."""
    from repowise.core.providers.embedding.base import EmbedderConfigError

    monkeypatch.setenv("OLLAMA_EMBEDDING_MODEL", "qwen3-embedding:0.6b")
    monkeypatch.setenv("OLLAMA_EMBEDDING_TIMEOUT", "not-a-number")
    with pytest.raises(EmbedderConfigError):
        providers.build_embedder("ollama")


def test_build_embedder_ollama_defaults_timeout_when_unset(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from repowise.core.providers.embedding.ollama import OllamaEmbedder
